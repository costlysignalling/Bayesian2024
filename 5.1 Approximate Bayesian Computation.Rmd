---
title: "9 Approximate Bayesian Computation"
author: "Jan Smyčka"
date: "08/01/2023"
...
The Approximate Bayesian Population is based on the idea that we simulate x values using some prior distribution of $\theta$ and use a meaningful summary statistic s() to transform x. Then we select simulation runs where s($x_obs$)-s($x_sim$) < tol. It is possible to prove that if we decrease tol -> 0, the $\theta$ values of selected simulation runs converge to the posterior. 
...
library(untb)
set.seed(13)
target <- untb(start=rep(1,10),prob=0.3, gens=50,keep=TRUE)
target[1:10,]
```
...
untb.streamlinedcurve=function(p){
  a=untb(start=rep(1,10),prob=p, gens=50,keep=TRUE)
...
target.fin=target.curve[1]
untbsummarystatsfin=untbsummarystatscurve[1,]
...
And what about using not the diversity curves, but the full abundance data?

```{r}
targetvec=as.vector(target)
targetvec

untb.streamlined=function(p){
  a=untb(start=rep(1,10),prob=p, gens=50,keep=TRUE)
  return(as.vector(a))
}
```
